...
namespace SeniorConnect.DAL
{
    public class ActivityLogic : BaseLogic
    {
        ...
        public List<User> GetUsers(int activityId)
        {
            if (activityId == 0)
            ...
            {
                conn.Open();
                var cmd = new MySqlCommand(@"SELECT * FROM chosenactivities ca JOIN users u ON ca.userId = u.userId WHERE ca.activityID = @activityid", conn);
